import pandas as pd
import re


class ProkkaParser:
    def __init__(self, gff_path, blast_path):
        self.gff_path = gff_path
        self.blast_path = blast_path
        self.merged_df = None

    def parse_gff_and_blast(self):
        lines = []
        with open(self.gff_path) as f:
            for line in f:
                if line.startswith("##FASTA"):
                    break
                if line.startswith("#"):
                    continue
                lines.append(line)

        df = pd.read_csv(
            pd.io.common.StringIO("".join(lines)),
            sep="\t",
            header=None,
            names=["seqid", "source", "type", "start", "end",
                   "score", "strand", "phase", "attributes"],
        )

        # Pull common fields from the attributes string
        df["gene"]       = df["attributes"].apply(lambda x: self.get_attr(x, "gene"))
        df["product"]    = df["attributes"].apply(lambda x: self.get_attr(x, "product"))
        df["locus_tag"]  = df["attributes"].apply(lambda x: self.get_attr(x, "locus_tag"))
        df["ID"]         = df["attributes"].apply(lambda x: self.get_attr(x, "ID"))

        ab = df["attributes"].apply(lambda x: self.get_attr(x, "database_protein.fasta"))
        df["protein_name"] = [x.split(":")[4] if isinstance(x, str) else "" for x in ab]
        df["custom_db"]    = [x.split(":")[3] if isinstance(x, str) else "" for x in ab]

        # Split CDS vs non-CDS — use .copy() to avoid SettingWithCopyWarning
        only_cds_df = df[df["type"] == "CDS"].copy().reset_index(drop=True)
        not_cds_df  = df[df["type"] != "CDS"].copy().reset_index(drop=True)

        only_cds_df["query_index"] = only_cds_df.index

        df_blast = self.parse_blast_to_dataframe()

        merged_df = only_cds_df.merge(df_blast, on="query_index", how="left")

        # Build not_cds rows with all columns, filled with pd.NA
        # Use reindex so no column-by-column assignment is needed (avoids warning + FutureWarning)
        not_cds_df = (
            not_cds_df
            .reindex(columns=merged_df.columns)   # adds missing cols as NaN in one shot
        )

        # Concatenate — both frames now have identical columns and proper dtypes
        final_df = pd.concat([merged_df, not_cds_df], ignore_index=True)

        return final_df

    def get_attr(self, attr_str, key):
        if pd.isna(attr_str):
            return None
        for item in str(attr_str).split(";"):
            if item.startswith(key + "=") or key in item:
                return item.split("=", 1)[1]
        return None

    def parse_blast_to_dataframe(self):
        """
        Parse BLASTP text output into a pandas DataFrame.

        Columns:
            query_index  : int  (0-based query number)
            identity_pct : int  (percentage identity)
            protein_id   : str  (FASTA header ID)
        """
        rows = []

        with open(self.blast_path, "r") as f:
            lines = f.readlines()

        current_query = None
        current_header = None
        collecting_header = False

        for line in lines:
            line = line.rstrip()

            if line.startswith("Query="):
                q = line.split("=", 1)[1].strip()
                current_query = int(q) - 1 if q.isdigit() else None
                current_header = None
                collecting_header = False
                continue

            if line.startswith(">"):
                current_header = line[1:].strip()
                collecting_header = True
                continue

            if collecting_header:
                if line.startswith("Length="):
                    collecting_header = False
                elif line.strip():
                    current_header += " " + line.strip()
                continue

            if "Identities =" in line and current_header is not None and current_query is not None:
                match = re.search(r"\((\d+)%\)", line)
                if match:
                    rows.append({
                        "query_index": current_query,
                        "identity_pct": int(match.group(1)),
                        "protein_id": current_header.split(" ")[0],
                    })
                    current_header = None

        return pd.DataFrame(rows, columns=["query_index", "identity_pct", "protein_id"])
